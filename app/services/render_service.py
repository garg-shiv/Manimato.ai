...
def cleanup_manim_files(script_filename: str):
    ...
    script_path = os.path.join(generated_dir, f"{script_filename}.py")
    ...
    if os.path.exists(pycache_dir):
        for cache_file in os.listdir(pycache_dir):
            if cache_file.startswith(
                f"{script_filename}.cpython-"
            ) and cache_file.endswith(".pyc"):
            ...
            for file in files:
                if file == f"{script_filename}.mp4":
                    ...
                    final_output_path = os.path.join(
                        media_dir, f"{script_filename}_output.mp4"
                    )
